    ...
    const lxUtils = {
        ...
        crypto: {
            ...
            aesEncrypt: (buffer: any, mode: string, key: any, iv: any) => {
                ...
                // Custom-source API passes either short modes (cbc/ecb) or full algorithms (aes-128-ecb).
                const normalizedMode = String(mode || "").toLowerCase()
                const algorithm = normalizedMode.startsWith("aes-")
                    ? normalizedMode
                    : `aes-${(dKey as any).length * 8}-${normalizedMode}`
